...
// The router will be added as a middleware and will take control of requests starting with path /room.
...
router.patch("/:roomName", async (req, res) => {
  try {
    const query =  { roomname: new ObjectId(req.params.roomName) };
    const updates = {
      $set: {
        roomname: req.body.name,
        accessibility: req.body.accessibility,
